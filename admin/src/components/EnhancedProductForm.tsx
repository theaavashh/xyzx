...
export default function EnhancedProductForm({
  ...
  useEffect(() => {
    ...
    const newVariants: GeneratedVariant[] = combinations.map((combination) => {
      ...
      const getOptionName = (attrId: string, val: string) => {
        const attr = variantAttributes.find((a) => a.id === attrId);
        const option = attr?.options.find((o) => o.id === val || (o as any).value === val);
        return option?.name || val;
      };

      const name = Object.entries(combination)
        .map(([attrId, val]) => getOptionName(attrId, val))
        .join(' / ');
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Success Message */}
        {showSuccess && (
          <div className="mx-6 mb-0 bg-emerald-50 border border-emerald-200 px-4 py-3">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-hidden flex flex-col">
          <div className="flex-1 overflow-y-auto px-6 py-5 max-h-[520px] scrollbar-hide">
            {activeTab === 'identity' && (
              <BasicInfoTab
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-gray-100 flex-shrink-0 bg-gray-50/50">
          <div className="flex items-center gap-2">
            <button type="button" onClick={onClose} className="px-3 py-1.5 text-xs font-medium text-gray-500 hover:text-gray-700 transition-colors">
              Cancel
            </button>
            {!isFirstTab && (
              <button type="button" onClick={handlePrevious} className="px-3 py-1.5 text-xs font-medium text-gray-500 hover:text-gray-700 transition-colors flex items-center gap-1">
          ...
          <div className="flex items-center gap-2">
            {!isLastTab && (
              <button type="button" onClick={handleNext} className="px-4 py-1.5 text-xs font-medium text-white bg-[#D4AF37] hover:bg-[#b8962e] transition-colors flex items-center gap-1">
                Next
            ...
            {isLastTab && (
              <button type="submit" onClick={handleSubmit} disabled={isLoading} className="px-4 py-1.5 text-xs font-medium text-white bg-[#D4AF37] hover:bg-[#b8962e] transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1.5">
                {isLoading ? (
                  <>
