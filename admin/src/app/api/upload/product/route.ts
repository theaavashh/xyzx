...
export async function POST(request: NextRequest) {
  try {
    ...
    // Build headers — let fetch auto-set Content-Type with the correct
    // multipart boundary for FormData; only include auth-related headers.
    ...
    try {
      response = await fetch(`${apiBaseUrl}/api/v1/upload/product`, {
        method: 'POST',
        headers,
        body: formData,
