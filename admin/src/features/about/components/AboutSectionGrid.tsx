  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {sections.map((section) => (
        ...
        >
          <p className="text-sm text-gray-700 italic mb-2 line-clamp-2">&ldquo;{section.quote}&rdquo;</p>
          {section.heroSubtitle && (
            <p className="text-xs text-gray-500 mb-1 line-clamp-1">Hero: {section.heroSubtitle}</p>
          )}
          {section.storyTitle && (
            <p className="text-xs text-gray-500 mb-1 line-clamp-1">Story: {section.storyTitle}</p>
          )}
          {section.storeAddress && (
            <p className="text-xs text-gray-500 mb-2 line-clamp-1">Store: {section.storeAddress}{section.storeCity ? `, ${section.storeCity}` : ''}</p>
          )}
