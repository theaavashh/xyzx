...
import type { ImageGridItem } from '../types';

interface ImageGridGridProps {
  items: ImageGridItem[];
  isLoading: boolean;
  onEdit: (item: ImageGridItem) => void;
  onToggle: (item: ImageGridItem) => void;
  onDelete: (item: ImageGridItem) => void;
  onReorder: (id: string, direction: 'up' | 'down') => void;
  onAdd: () => void;
}

export function ImageGridGrid({
  items,
  isLoading,
  onEdit,
  onToggle,
    ...
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-lg font-medium text-gray-900 mb-2">
          No image grid items found
        </h3>
        <p className="text-gray-600 mb-4">
          Get started by creating your first image grid item
        </p>
        <button
          type="button"
          onClick={onAdd}
