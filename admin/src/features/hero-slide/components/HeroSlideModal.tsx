...
export function HeroSlideModal({
  ...
  const [form, setForm] = useState<HeroSlideForm>({
    title: '', subtitle: '', image: '', imageMobile: '', alt: '', isActive: true, order: 0,
  });
  ...
  useEffect(() => {
    if (isOpen) {
      if (editingItem) {
        ...
        setForm({ title: '', subtitle: '', image: '', imageMobile: '', alt: '', isActive: true, order: 0 });
      }
    }
  }, [isOpen, editingItem]);
    ...
    <div>
      ...
      {isUploading ? (
        ...
        <div className="relative">
          <img src={value} alt="Preview" className="w-full h-48 object-cover rounded-lg border border-gray-200 bg-gray-100" crossOrigin="anonymous"
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          className="fixed inset-0 bg-black/50 flex items-end justify-center z-50 sm:items-center p-4" onClick={onClose}>
            ...
            <form onSubmit={handleSubmit} className="space-y-6 p-6">
              <div>
                ...
                <input id="title" type="text" value={form.title} onChange={(e) => handleChange('title', e.target.value)}
                  placeholder="Enter slide title" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#D4AF37] text-black" required />
              </div>
              <div>
                <label htmlFor="subtitle" className="block text-sm font-medium text-gray-700 mb-1">Subtitle</label>
                <input id="subtitle" type="text" value={form.subtitle} onChange={(e) => handleChange('subtitle', e.target.value)}
                  placeholder="e.g., Everyday pieces in easy, wearable fabrics" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#D4AF37] text-black" />
              </div>
              ...
              <div>
                ...
                <input id="alt" type="text" value={form.alt} onChange={(e) => handleChange('alt', e.target.value)}
                  placeholder="e.g., Fashion model wearing elegant everyday clothing" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#D4AF37] text-black" />
              </div>
