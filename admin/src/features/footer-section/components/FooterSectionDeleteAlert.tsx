...
interface FooterSectionDeleteAlertProps {
  show: boolean;
...
export function FooterSectionDeleteAlert({
  show,
  section,
  onConfirm,
  onCancel,
}: FooterSectionDeleteAlertProps) {
  return (
    <AnimatePresence>
      {show && section && (
        <motion.div
        ...
        >
          ...
          >
            ...
            <p className="text-gray-400 text-center mb-6 text-xs">
              This action cannot be undone. All links in this section will also be removed.
            </p>
            <div className="flex gap-3">
              <button
                type="button"
                onClick={onConfirm}
                className="flex-1 bg-red-600 text-white px-4 py-2.5 rounded-lg hover:bg-red-700 transition-colors font-medium text-sm"
              >
                Delete
              </button>
              <button
