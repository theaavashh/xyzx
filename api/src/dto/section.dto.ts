...
export const heroBannerCreateSchema = z.object({
  ...
  isActive: z.boolean().default(true),
  order: z.number().int().min(0).default(0),
  position: z.enum(['TOP_LEFT', 'TOP_RIGHT', 'BOTTOM_LEFT', 'BOTTOM_RIGHT', 'CENTER']).default('CENTER'),
}).refine(
  (data) => data.largeImage || data.videoUrl,
  { message: 'Either desktop image or video URL is required', path: ['largeImage'] },
);
