...
async function createAdminAndSendCredentials() {
  try {
    for (const { email, name, password: presetPassword } of admins) {
      const password = presetPassword || Math.random().toString(36).slice(-8) + Math.random().toString(36).slice(-8).toUpperCase();
      const hashedPassword = await bcrypt.hash(password, 10);

      const existing = await prisma.user.findUnique({ where: { email } });
      if (existing) {
        console.log(`User ${email} already exists. Updating role, password, and sending new credentials...`);
