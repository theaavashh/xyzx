...
const EMAIL = 'pmanish2055@gmail.com';

function generatePassword(): string {
  const upper = crypto.randomBytes(4).toString('hex').toUpperCase();
  const lower = crypto.randomBytes(4).toString('hex').toLowerCase();
  const digits = crypto.randomInt(1000, 9999).toString();
  return upper.slice(0, 4) + lower.slice(0, 4) + digits;
}
...
async function sendNewCredential() {
  try {
                ...
                <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.1);">
                  ...
                  <tr>
                    <td style="padding:40px;">
                      <h2 style="color:#1a1a1a;margin:0 0 16px;font-size:20px;">Your New Admin Credentials</h2>
                      <p style="color:#555555;font-size:15px;line-height:1.6;margin:0 0 24px;">
                        Your admin credentials have been reset. Here are your new login details:
                      </p>
    ...
    if (error) {
      ...
      console.log('New password (not sent via email):', password);
    } else {
